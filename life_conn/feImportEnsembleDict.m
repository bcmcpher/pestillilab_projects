function [ fgOut, dict ] = feImportEnsembleDict(ens_tck, fout)
...
for ii = 1:length(1:10:length(files))
    ...
    for jj = 1:10
        ...
        if count > 0
            ...
            % open fibers as object
            fg = dtiImportFibersMrtrix(fullfile(ens_tck, tmp));
            
            % quickly merge them to output fiber group
            fgOut.fibers = vertcat(fgOut.fibers, fg.fibers);
...
% write down fgOut
display(['Writing .mat for file ' fout]);
