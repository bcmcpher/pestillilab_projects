function [ emat, nmat, fns, pval, fdat, imat, time ] = feMatrixFromTensor(subj, dgrp, dmdl, lmax) 
...
% subject index
% Stanford Data Order: 'FP', 'MP', 'HT', 'KK', 'KW'
% HCP Data Order: '105115', '110411', '111312', '113619'
file.name.indx = subj;

% run 'stanford' or 'hcp' data
dataGroup = dgrp;
...
addpath(genpath('/N/dc2/projects/lifebid/code_ht/life/external/emd'));
addpath(genpath('~/Toolboxes/BCT'));
addpath(genpath('~/Toolboxes/NBS'));
addpath(genpath('N/dc2/projects/lifebid/HCP/Brent/cogs610/matlab'));

clear vista_soft_path mba_soft_path new_LiFE_path RESTOREDEFAULTPATH_EXECUTED
...
file.name.lmax = 'lmax10';

% output directory for saved network results
file.out.dir = '/N/dc2/projects/lifebid/HCP/Brent/cogs610/ep_data';
...
file.stn.path.prob = char(fullfile(file.dir.path, file.stn.name.dir(file.name.indx), file.stn.name.prob.sngl));
file.stn.path.detr = char(fullfile(file.dir.path, file.stn.name.dir(file.name.indx), file.stn.name.detr.sngl));
file.stn.path.tens = char(fullfile(file.dir.path, file.stn.name.dir(file.name.indx), file.stn.name.tens.sngl));

file.hcp.path.anat = char(fullfile(file.hcp.dir.anat(file.name.indx), file.name.anat));
file.hcp.path.prob = char(fullfile(file.dir.path, file.hcp.name.dir(file.name.indx), file.hcp.name.prob.sngl));
file.hcp.path.detr = char(fullfile(file.dir.path, file.hcp.name.dir(file.name.indx), file.hcp.name.detr.sngl));
file.hcp.path.tens = char(fullfile(file.dir.path, file.hcp.name.dir(file.name.indx), file.hcp.name.tens.sngl));
...
% % should be able to target duplicate fibers with overlapping voxels to
% % prevent full iteration through every combo or regions
% for ii = 1:length(pairs)
%     if pconn{ii}.intersect == 1
%         % remove fibers in intersecting voxels
%     end
% end
...
for ii = 1:length(pconn)
    for jj = 1:length(pconn)
        % don't compare to the same one or if voxels don't overlap
        if ii ~= jj %&& size(intersect(pconn{ii}.roisvx, pconn{jj}.roisvx, 'rows'), 1) > 0
            
            % find indices of each non overlapping int fiber and keep just the unique indices
            [ ~, pt1, pt2 ] = setxor(pconn{ii}.int.fibers, pconn{jj}.int.fibers);
            pconn{ii}.int.fibers = pconn{ii}.int.fibers(pt1);
            pconn{ii}.int.lengths = pconn{ii}.int.lengths(pt1);
            pconn{ii}.int.weights = pconn{ii}.int.weights(pt1);
            pconn{jj}.int.fibers = pconn{jj}.int.fibers(pt2);
            pconn{jj}.int.lengths = pconn{jj}.int.lengths(pt2);
            pconn{jj}.int.weights = pconn{jj}.int.weights(pt2);
            
            [ ~, pt1, pt2 ] = setxor(pconn{ii}.int.nzfibs, pconn{jj}.int.nzfibs);
            pconn{ii}.int.nzfibs = pconn{ii}.int.nzfibs(pt1);
            pconn{ii}.int.nzleng = pconn{ii}.int.nzleng(pt1);
            pconn{ii}.int.nzwght = pconn{ii}.int.nzwght(pt1);
            pconn{jj}.int.nzfibs = pconn{jj}.int.nzfibs(pt2);
            pconn{jj}.int.nzleng = pconn{jj}.int.nzleng(pt2);
            pconn{jj}.int.nzwght = pconn{jj}.int.nzwght(pt2);
            
            % find indices of each non overlapping end fiber and keep just the unique indices
            [ ~, pt1, pt2 ] = setxor(pconn{ii}.end.fibers, pconn{jj}.end.fibers);
            pconn{ii}.end.fibers = pconn{ii}.end.fibers(pt1);
            pconn{ii}.end.lengths = pconn{ii}.end.lengths(pt1);
            pconn{ii}.end.weights = pconn{ii}.end.weights(pt1);
            pconn{jj}.end.fibers = pconn{jj}.end.fibers(pt2);
            pconn{jj}.end.lengths = pconn{jj}.end.lengths(pt2);
            pconn{jj}.end.weights = pconn{jj}.end.weights(pt2);
            
            [ ~, pt1, pt2 ] = setxor(pconn{ii}.end.nzfibs, pconn{jj}.end.nzfibs);
            pconn{ii}.end.nzfibs = pconn{ii}.end.nzfibs(pt1);
            pconn{ii}.end.nzleng = pconn{ii}.end.nzleng(pt1);
            pconn{ii}.end.nzwght = pconn{ii}.end.nzwght(pt1);
            pconn{jj}.end.nzfibs = pconn{jj}.end.nzfibs(pt2);
            pconn{jj}.end.nzleng = pconn{jj}.end.nzleng(pt2);
            pconn{jj}.end.nzwght = pconn{jj}.end.nzwght(pt2);
            
            % find indices of each non overlapping nid1 fiber and keep just the unique indices
            [ ~, pt1, pt2 ] = setxor(pconn{ii}.nid1.fibers, pconn{jj}.nid1.fibers);
            pconn{ii}.nid1.fibers = pconn{ii}.nid1.fibers(pt1);
            pconn{ii}.nid1.lengths = pconn{ii}.nid1.lengths(pt1);
            pconn{ii}.nid1.weights = pconn{ii}.nid1.weights(pt1);
            pconn{jj}.nid1.fibers = pconn{jj}.nid1.fibers(pt2);
            pconn{jj}.nid1.lengths = pconn{jj}.nid1.lengths(pt2);
            pconn{jj}.nid1.weights = pconn{jj}.nid1.weights(pt2);
            
            [ ~, pt1, pt2 ] = setxor(pconn{ii}.nid1.nzfibs, pconn{jj}.nid1.nzfibs);
            pconn{ii}.nid1.nzfibs = pconn{ii}.nid1.nzfibs(pt1);
            pconn{ii}.nid1.nzleng = pconn{ii}.nid1.nzleng(pt1);
            pconn{ii}.nid1.nzwght = pconn{ii}.nid1.nzwght(pt1);
            pconn{jj}.nid1.nzfibs = pconn{jj}.nid1.nzfibs(pt2);
            pconn{jj}.nid1.nzleng = pconn{jj}.nid1.nzleng(pt2);
            pconn{jj}.nid1.nzwght = pconn{jj}.nid1.nzwght(pt2);
            
            % find indices of each non overlapping nid2 fiber and keep just the unique indices
            [ ~, pt1, pt2 ] = setxor(pconn{ii}.nid2.fibers, pconn{jj}.nid2.fibers);
            pconn{ii}.nid2.fibers = pconn{ii}.nid2.fibers(pt1);
            pconn{ii}.nid2.lengths = pconn{ii}.nid2.lengths(pt1);
            pconn{ii}.nid2.weights = pconn{ii}.nid2.weights(pt1);
            pconn{jj}.nid2.fibers = pconn{jj}.nid2.fibers(pt2);
            pconn{jj}.nid2.lengths = pconn{jj}.nid2.lengths(pt2);
            pconn{jj}.nid2.weights = pconn{jj}.nid2.weights(pt2);
            
            [ ~, pt1, pt2 ] = setxor(pconn{ii}.nid2.nzfibs, pconn{jj}.nid2.nzfibs);
            pconn{ii}.nid2.nzfibs = pconn{ii}.nid2.nzfibs(pt1);
            pconn{ii}.nid2.nzleng = pconn{ii}.nid2.nzleng(pt1);
            pconn{ii}.nid2.nzwght = pconn{ii}.nid2.nzwght(pt1);
            pconn{jj}.nid2.nzfibs = pconn{jj}.nid2.nzfibs(pt2);
            pconn{jj}.nid2.nzleng = pconn{jj}.nid2.nzleng(pt2);
            pconn{jj}.nid2.nzwght = pconn{jj}.nid2.nzwght(pt2);
            
        end
    end
end
time.dupchk = toc; % ~8 hours, checking if it's worth running takes longer?

% takes too long? should be faster
% iter = 1;
% for ii = 1:length(pconn)
%     for jj = 1:length(pconn)
%         % don't compare to the same one or if voxels don't overlap
...
% cfib = 0;
% for ii = 1:length(pconn)
%     cfib = cfib + length(pconn{ii}.end.fibers);
%     nfib = [nfib; pconn{ii}.end.fibers];
% end
% 
% % should be the same
% nfib = sort(nfib);
% ufib = unique(nfib);
% clear cfib nfib ufib
...
for ii = 1:length(pconn)
    try
        ...
        ifevl{ii} = feComputeEvidence(iwoVL, iwVL);
    ...
    try
        ...
        efevl{ii} = feComputeEvidence(ewoVL, ewVL);
...
switch dataGroup
    case 'stanford'
        saveNames = char(strcat(dataGroup, '_', file.stn.name.dir(file.name.indx), '_', dataModel, '_', file.name.lmax, '.mat'));
    case 'hcp'
        saveNames = char(strcat(dataGroup, '_', file.hcp.name.dir(file.name.indx), '_', dataModel, '_', file.name.lmax, '.mat'));
    otherwise
        error('No valid data group selected');
end
...
% for ii = 1:length(pairs)
%     
%     % create matrix output for intersection
%     cmat(pairs(ii, 1), pairs(ii, 2), 1) = length(pconn{ii}.int.fibers);
%     cmat(pairs(ii, 2), pairs(ii, 1), 1) = length(pconn{ii}.int.fibers);
% 
%     % create matrix output for endpoint connectivity
%     cmat(pairs(ii, 1), pairs(ii, 2), 2) = length(pconn{ii}.end.fibers);
%     cmat(pairs(ii, 2), pairs(ii, 1), 2) = length(pconn{ii}.end.fibers);
%     
%     % create matrix output for endpoint to non-endpoint connectivity
%     cmat(pairs(ii, 1), pairs(ii, 2), 3) = length(pconn{ii}.nid1.fibers);
%     cmat(pairs(ii, 2), pairs(ii, 1), 3) = length(pconn{ii}.nid2.fibers);
% 
% end
% 
% % plot the 3 figures
...
% colormap('hot');
% imagesc(log(cmat(:,:,3)));
% title('Endpoint to Intersection');
% xlabel('FS DK Regions');
% ylabel('FS DK Regions');
% axis('square'); axis('equal'); axis('tight');
% y = colorbar;
...
%% development code to not forget just yet

% % use single image of all ROIs to find connections to prevent overlap

% BECAUSE WM MASK USED IN FE IS BASED ON THIS IMAGE, WILL THERE BE NO OVERLAP?

% load aparc+aseg nifti of FreeSurfer output
%apas = niftiRead(fullfile(file.dir.anat, 'mri', 'aparc+aseg.nii.gz'));

% % pull all coordinates and xform them
% [ apasCoords(:, 1), apasCoords(:, 2), apasCoords(:, 3) ] = ind2sub(size(apas.data), apas.data(:));
% apasXCoords = mrAnatXformCoords(fe.life.xform.acpc2img, apasCoords);
% % way too small
% apasXCoords = unique(apasXCoords, 'rows');

% APPLY TRANSFORM TO RESAMPLE DATA MATRIX TO fe.roi.coords SPACE

% find the size of the (resampled) data matrix
%apasSize = size(apas.data);

% to be replaced in loop
% pull roiCoords for this label from resampled image
% loop over non-overlapping ROI labels in original image instead of
% distorted individual ROI boundaries
%[ roiCoords(:, 1), roiCoords(:, 2), roiCoords(:, 3) ] = ind2sub(apasSize, find(apas.data == 1005));

% roiCoords should overlap w/ ROI in fe structure space

% apply transform to ROI once coords are found - same problem?
%xformCoords = mrAnatXformCoords(fe.life.xform.acpc2img, roiCoords);
%xformCoords = ceil(xformCoords);
%xformCoords = unique(xformCoords, 'rows');

% no overlap in coordinates - not a small difference either...
%z = ismember(fe.roi.coords, xformCoords);


% % development of removing duplicate fibers
% tmp1 = pconn{1}.end;
% tmp2 = pconn{2}.end;
% tint = intersect(tmp1, tmp2);
% 
% [ txor, it1, it2 ] = setxor(tmp1, tmp2);
% ftmp1 = tmp1(it1);
% ftmp2 = tmp2(it2);
% 
% tmp3 = [tmp1; tmp2];
% tmp4 = unique(tmp3);
% ftmp3 = unique([ftmp1; ftmp2]);



% was inside loop

%     % find fiber endpoints in ep{out{ii}.raw.fibers} to keep
%     vx.ep = ep(out{ii}.raw.fibers, :);
%     
%     % stack all endpoints into a single unique vector
%     vx.epv = unique([vx.ep(:, 1); vx.ep(:, 2)]);
%     
%     % find intersect of ROI coords w/ endpoint coords 
%     vx.endp = ismember(vx.index, vx.epv, 'rows');
%     
%     % pull unique fiber endpoint vx indices into output object
%     [ inds2, ~ ] = find(fe.life.M.Phi(:, vx.endp, :));
%     
%     if size(inds2) > 0
%         out{ii}.raw.endp = unique(inds2(:, 3));
%     else
%         out{ii}.raw.endp = [];
%     end
    
%     outindices = zeros(1, length(out{ii}.raw.fibers));
%     
%     for jj = 1:length(out{ii}.raw.fibers)
%         
%         % pull beginning / end coordinates
%         beg = fe.fg.fibers{out{ii}.raw.fibers(jj)}(:, 1);
%         bck = fe.fg.fibers{out{ii}.raw.fibers(jj)}(:, end);
%         
%         % convert beginning / end fiber coordinates to voxel space
%         tbeg = mrAnatXformCoords(fe.life.xform.acpc2img, beg);
%         tbck = mrAnatXformCoords(fe.life.xform.acpc2img, bck);
%         
%         % find intersect of beginning / end with fe space
%         endindices = ismember(fe.roi.coords, [tbeg; tbck]);
%         
%         if endindices > 0
%             outindices(jj) = 1;
%         end
...
% tfib(:,1)
% mdst = pdist([tfib(:,1)'; fe.roi.coords]);
% 
% % tested on small subset; does what it's supposed to
% feTmp = fe.fg;
% feTmp.fibers = feTmp.fibers(1:100);
%
% % pull a fiber for testing
% tfib = fe.fg.fibers{1};
% 
% % euclidian distance between fiber end points
% pdist([tfib(:,1)'; tfib(:,end)'], 'euclidean')
% pdist([round(tfib(:,1))'; round(tfib(:,end))'], 'euclidean')
% 
% for ii = 1:length(fe.fg.fibers)
%     dmChk(ii,:) = minmax(fe.fg.fibers{ii});
% end

% % runs SLOW - would vectorized be faster?
% % find end point voxels of each fiber
% tic;
% for ii = 1:size(fe.life.M.Phi, 3)
%     fib = find(fe.life.M.Phi(:,:,ii));
%     
%     % find first and last index - assumes fiber nodes are in order in tensor
%     %ep{ii}.first = fib(1,2);
%     %ep{ii}.last  = fib(end,2);
%     % wrong; first fiber has 1 voxel of space, travels 6x15x5 mm
%     
%     % find max dist between nodes, assume those are endpoint voxels
%     % what about false fibers that loop around? reasonable?
%     vxOut = minmax(fib(:,2));
%     ep(ii, 1) = vxOut(1);
%     ep(ii, 2) = vxOut(2);
%     
%     % can i really not find endpoints of fibers in tensor?
%     
% end
% ep_time = toc;

% plot first non-zero fiber
%plot3(fe.fg.fibers{4}(1,:),fe.fg.fibers{4}(2,:),fe.fg.fibers{4}(3,:)); hold on;
%plot3(82.601, 95.1425, 35.2410, 'ro');
%plot3(71.2768, 98.9039, 42.8807, 'ro');
